...
// 基于 dreamina CLI v1.4.2+ 实际支持的 --model_version 参数：
//   text2video: seedance2.0 / seedance2.0fast
//   multimodal2video: seedance2.0 / seedance2.0fast
//   image2video: 3.0 / 3.0fast / 3.0pro / 3.5pro / seedance2.0 / seedance2.0fast
//   frames2video: 3.0 / 3.5pro / seedance2.0 / seedance2.0fast
//   multiframe2video: 不支持 --model_version (CLI 自动选择)
const JIMENG_SEEDANCE_VIDEO_MODELS = ['seedance2.0_vip', 'seedance2.0fast_vip', 'seedance2.0', 'seedance2.0fast'];
const JIMENG_IMAGE2VIDEO_MODELS = ['3.0', '3.0fast', '3.0pro', '3.5pro', ...JIMENG_SEEDANCE_VIDEO_MODELS];
const JIMENG_FRAMES2VIDEO_MODELS = ['3.0', '3.5pro', ...JIMENG_SEEDANCE_VIDEO_MODELS];
const JIMENG_VIDEO_MODELS_BY_COMMAND = {
    text2video: JIMENG_SEEDANCE_VIDEO_MODELS,
    multimodal2video: JIMENG_SEEDANCE_VIDEO_MODELS,
    image2video: JIMENG_IMAGE2VIDEO_MODELS,
    frames2video: JIMENG_FRAMES2VIDEO_MODELS,
    multiframe2video: [],  // CLI 不支持 --model_version，不过滤，让后端忽略该参数
};
...
function filterJimengVideoModels(models){
    ...
    // undefined = 命令未在白名单中，不过滤（兜底）
    if(!allowed) return models;
    // 空数组 = CLI 不支持 --model_version 参数，显示全部但提示
    if(allowed.length === 0) return (models || []).slice();
