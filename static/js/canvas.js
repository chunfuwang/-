...
const canvasPromptLibraryToggle = document.getElementById('canvasPromptLibraryToggle');
...
function providerVideoModels(providerId){
    // 优先从已启用且有 video_models 的平台列表中查找
...
function openCreateMenu(clientX, clientY){
    ...
    closeLinkCreateMenu();
    closeAddNodePalette();
    createMenu.classList.toggle('submenu-left', clientX + 570 > window.innerWidth);
    createMenu.style.left = `${clientX}px`;
    createMenu.style.top = `${clientY}px`;
    createMenu.classList.add('open');
    refreshIcons();
}
function closeAddNodePalette(){
    addNodePalette?.classList.remove('open');
    addNodeMenuToggle?.classList.remove('active');
    addNodeMenuToggle?.setAttribute('aria-expanded', 'false');
}
function toggleAddNodePalette(e){
    e?.preventDefault();
    e?.stopPropagation();
    ...
    addNodeMenuToggle?.setAttribute('aria-expanded', 'true');
    refreshIcons();
}
...
addNodeMenuToggle?.addEventListener('click', toggleAddNodePalette);
addNodePalette?.querySelectorAll('[data-add-node-placeholder]').forEach(item => {
...
function bindOutputWrap(wrap, node){
    ...
    if(playBtn){
        ...
        playBtn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            // 动态查找当前媒体元素：预览图加载成功时为 img，失败 fallback 后为 video
            const vImg = wrap.querySelector('img[data-preview-kind="video"]');
            if(vImg && canvasActivateVideoPreview(vImg)) return;
            const vEl = wrap.querySelector('video');
            if(vEl){
                openOutputLightbox(vEl.dataset.url || wrap.dataset.outputUrl, node);
            } else if(img){
                openOutputLightbox(img.dataset.url || wrap.dataset.outputUrl, node);
            }
        };
    }
    if(recoverQuery){
...
async function openPromptTemplateModal(nodeId){
    ...
    canvasPromptLibraryToggle?.classList.add('active');
...
function closePromptTemplateModal(){
    ...
    canvasPromptLibraryToggle?.classList.remove('active');
...
canvasAssetToggle?.addEventListener('click', () => {
    if(promptTemplateModal?.classList.contains('open')) closePromptTemplateModal();
    toggleCanvasAssetLibrary();
});
canvasPromptLibraryToggle?.addEventListener('click', async () => {
    if(canvasPromptLibraryToggle.hasAttribute('onclick')) return;
    if(promptTemplateModal?.classList.contains('open')){
        closePromptTemplateModal();
        return;
    }
    toggleCanvasAssetLibrary(false);
    await openPromptTemplateModal('');
});
