...
# 即梦 CLI 各视频命令支持的模型版本（基于 dreamina CLI v1.4.2+ help）
# text2video / multimodal2video: seedance2.0, seedance2.0fast
# image2video: 3.0, 3.0fast, 3.0pro, 3.5pro, seedance2.0, seedance2.0fast
# frames2video: 3.0, 3.5pro, seedance2.0, seedance2.0fast
# multiframe2video: 不支持 --model_version
JIMENG_VIDEO_COMMANDS_WITH_MODEL = {"text2video", "multimodal2video", "image2video", "frames2video"}
...
def api_headers(json_body=True, provider=None, model=""):
    ...
    elif provider and is_mimo_provider(provider):
...
def is_mimo_provider(provider):
    """小米 MiMo API 使用 api-key Header 认证而非 Authorization: Bearer"""
    base_url = str((provider or {}).get("base_url") or "").lower()
    return "xiaomimimo.com" in base_url or str((provider or {}).get("id") or "").strip().lower() == "mimo"
...
def jimeng_video_model_version(model):
    ...
    low = value.lower()
    aliases = {
        # VIP 模型映射到 CLI 实际支持的 model_version
        "seedance2.0fast_vip": "seedance2.0fast",
        "seedance2.0_vip": "seedance2.0",
        "seedance2.0fast": "seedance2.0fast",
        "seedance2.0": "seedance2.0",
        # 兼容火山引擎 doubao-seedance 格式（带连字符）→ 即梦 CLI 格式
        ...
        "3.5_pro": "3.5pro",
        "3.5pro": "3.5pro",
        # 兼容旧版/手写变体必须在通用短键 "3.0" 之前，避免子串误匹配
        "jimeng-video-3.0-pro": "3.0pro",
        "jimeng-video-3.0": "3.0",
        "jimeng-vide-3.0": "3.0",
        "3.0": "3.0",
        # 即梦网页新增模型 Seedance 1.5 Pro / 1.0+ / 1.0 Fast
        "seedance-1.5-pro": "seedance1.5pro",
        "seedance1.5pro": "seedance1.5pro",
        "seedance-1.0-plus": "seedance1.0",
    ...
    try:
        ...
        if refs:
            ...
            for path in image_paths:
                args.append(f"--images={jimeng_cli_path_arg(path)}")
            args.extend([
                f"--prompt={prompt}",
    ...
    try:
        if payload.multimodal or video_refs or audio_refs:
            ...
            if first_frame and last_frame:
                ...
                # multiframe2video 不支持 --model_version / --video_resolution 参数
                # CLI 自动根据输入图片推断最佳模型
        elif image_refs:
            ...
            ratio = jimeng_video_ratio_arg(payload.aspect_ratio)
            model_version = jimeng_video_model_version(payload.model)
            is_seedance = model_version.startswith("seedance")
            if ratio and is_seedance:
                args = [
                    "multimodal2video",
...
@app.get("/")
async def root():
    return static_html_response("splash.html")
...
class WerewolfChatRequest(BaseModel):
    provider_id: str
    model: str
    system_prompt: str
    user_message: str
...
@app.post("/api/werewolf/chat")
async def werewolf_chat(payload: WerewolfChatRequest):
    provider = get_api_provider(payload.provider_id)
    base_root = (provider.get("base_url") or "").rstrip("/")
    if not base_root:
        raise HTTPException(status_code=400, detail=f"{provider.get('name') or provider['id']} 未配置 Base URL")
    try:
        chat_base, chat_hdrs, model = resolve_chat_provider(provider["id"], payload.model, "")
...
# 智能体兜底 API（当用户未配置 provider 时使用）
AGENT_FALLBACK_BASE = os.getenv("AGENT_FALLBACK_BASE", "https://svip.fluapi.com/v1")
AGENT_FALLBACK_KEY = os.getenv("AGENT_FALLBACK_KEY", "")

def _resolve_agent_provider(provider_id: str):
    """根据 provider_id 查找 base_url 和 api_key。未指定或找不到时使用兜底。"""
    if provider_id:
        providers = load_api_providers()
        for p in providers:
    ...
    # fallback
    fallback_key = AGENT_FALLBACK_KEY or os.getenv("COMFLY_API_KEY", "")
    return AGENT_FALLBACK_BASE, fallback_key

@app.post("/api/agent/chat")
async def agent_chat(payload: AgentChatRequest):
    base_url, api_key = _resolve_agent_provider(payload.provider_id)
    if not api_key:
        raise HTTPException(status_code=503, detail="未配置 API Key，请先在 API 设置中保存平台 Key")
    ...
    system = "\n".join(system_parts) if system_parts else "你是一个专业、友好的AI助手。"
    ...
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            is_mimo = payload.provider_id and payload.provider_id.strip().lower() == "mimo"
            agent_hdrs = {
                "api-key": api_key,
                "Content-Type": "application/json",
            } if is_mimo else {
            ...
            return {"reply": reply, "model": payload.model, "provider_id": payload.provider_id}
